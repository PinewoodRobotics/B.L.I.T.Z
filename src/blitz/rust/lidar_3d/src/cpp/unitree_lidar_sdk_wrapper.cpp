...
struct Imu
{
    ...
    float quaternion[4];          // quaternion: [x,y,z,w] - fixed array
    float angular_velocity[3];    // three-axis angular velocity values - fixed array
    float linear_acceleration[3]; // three-axis acceleration values - fixed array
};

// Create a wrapper struct to store the LiDAR instance
extern "C" {

void getImu(UnitreeLidarReader *reader, Imu *imu)
{
    if (!reader || !imu)
        return;

    IMUUnitree imuCpp = reader->getIMU();
    imu->stamp = imuCpp.stamp;
    imu->id = imuCpp.id;

    // Copy the arrays instead of assigning pointers
    for (int i = 0; i < 4; i++) {
        imu->quaternion[i] = imuCpp.quaternion[i];
    }
    for (int i = 0; i < 3; i++) {
        imu->angular_velocity[i] = imuCpp.angular_velocity[i];
        imu->linear_acceleration[i] = imuCpp.linear_acceleration[i];
    }
}
...
int initialize(UnitreeLidarReader *reader,
               uint16_t cloud_scan_num,
               const char *port,
               uint32_t baudrate,
               float rotate_yaw_bias,
               float range_scale,
               float range_bias,
               float range_max,
               float range_min)
{
    if (!reader)
        return -1;
    try {
        return reader->initialize(cloud_scan_num,
                                  std::string(port),
                                  baudrate,
                                  rotate_yaw_bias,
                                  range_scale,
...
void getVersionOfFirmware(UnitreeLidarReader *reader, char *buffer, size_t buffer_size)
{
    if (!reader || !buffer)
        return;
...
// Get SDK version
void getVersionOfSDK(UnitreeLidarReader *reader, char *buffer, size_t buffer_size)
{
    if (!reader || !buffer)
        return;
