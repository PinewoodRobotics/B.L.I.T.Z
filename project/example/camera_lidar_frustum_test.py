...
class LidarPointsFrustum:
    ...
    def get_frustum_points(self, origin: Point3D, other: list[Point3D]):
        ...
        json_data = {
            ...
            "directions": [
                {"x": p.x, "y": p.y, "z": p.z} for p in other
            ]
        }

        response = requests.post(f"http://localhost:{self.port}/post-json", json=json_data, headers=headers)
...
def render_frustum_and_points_with_axes(frustum_points, lidar_points):
    ...
    frustum_lines = [
        ...
        [0, 4], [1, 4], [2, 4], [3, 4]   # Sides connecting to apex
    ]
...
async def main():
    ...
    camera_matrix = np.array([ # for small black camera
    ...
    try:
        while True:
            ...
            for inf in inference.inferences:
                ...
                frustum_lineset.points = o3d.utility.Vector3dVector(np.array(frustum_points))
