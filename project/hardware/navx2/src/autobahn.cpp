...
void Autobahn::subscribe(const std::string& topic, std::function<void(const std::string&)> callback) {
...
void Autobahn::parse_message(const std::string& message) {
    try {
        ...
        // Read topic length
        uint16_t topic_length = static_cast<uint8_t>(*it++) << 8 | static_cast<uint8_t>(*it++);
        ...
        if (flag == Flags::PUBLISH) {
            for (const auto& [subscription_topic, callback] : subscriptions_) {
                if (topic.compare(0, subscription_topic.size(), subscription_topic) == 0) {
                    callback(payload);
                }
            }
        }
...
void Autobahn::send_message(Flags flag, const std::string& topic, const std::string& message) {
    try {
