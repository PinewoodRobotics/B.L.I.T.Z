...
from project.common.camera.box_fov_conversion import bbox_to_frustum, transform_frustum
    ...
    try:
        while True:
            ...
            for inference in inference.inferences:
                transformed_frustum = bbox_to_frustum(camera_matrix, inference.bounding_box, 5)
                res = lidar_frustum.get_frustum_points(Point3D(-0.012, 0, 0), [Point3D(pt[0], pt[1], pt[2]) for pt in transformed_frustum])
                if res == None:
                    print("Womp Womp")
                else:
                    ...
                    for i in res:
                        avg_total_distance = avg_total_distance + np.sqrt(i.x * i.x + i.y * i.y + i.z * i.z)
                    print(len(res))
